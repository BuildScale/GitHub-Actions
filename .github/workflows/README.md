...
jobs:
  docker_build:
    ...
    with:
      repo: buildscale/bash-tools  # DockerHub user/repo
      tags: latest ubuntu          # builds, tags as buildscale/bash-tools:latest and buildscale/bash-tools:ubuntu and pushes to DockerHub
      context: devops-scripts-ubuntu  # path to dir containing the source and Dockerfile
...
Supports building + pushing to any combination of the following, just add the relevant secrets, see [docker_build.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/docker_build.yaml) for details:
...
jobs:
  docker_build:
    ...
    with:
      repo_tags: |
        buildscale/bash-tools:latest
        ghcr.io/buildscale/bash-tools:latest
      context: devops-scripts-ubuntu  # path to dir containing the source and Dockerfile
...
- [checkov.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/checkov.yaml)
- [kics.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/kics.yaml)
- [semgrep.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/semgrep.yaml)
- [tfsec.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/tfsec.yaml)
- [trivy.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/trivy.yaml)


## Linting Auto-fixers

For workflows that lint-and-fix code, such as [terraform-fmt-write.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/terraform-fmt-write.yaml), you'll need to grant:
```yaml
permissions:
...
For workflows that create or comment on PRs, such as [terraform.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/terraform.yaml) and [tfsec-pr-commenter.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/tfsec-pr-commenter.yaml) you'll need to grant:
```yaml
permissions:
  contents: read
  pull-requests: write
...
For workflows that merge PRs, such as [merge-branch.yaml](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/merge-branch.yaml) you'll need to grant:
```yaml
permissions:
  contents: write
  pull-requests: write
...
You can create environment branches in your forked repo to stage updates across dev/staging/production and enable the [fork-sync](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/fork-sync.yaml) github actions workflow in your fork to keep the master branch sync'd every few hours, and the [fork-update-pr](https://github.com/BuildScale/GitHub-Actions/blob/master/.github/workflows/fork-update-pr.yaml) github actions workflow to raise GitHub Pull Requests for your environment branches to audit, authorize & control updates.
